...
OUT="logs/$BENCHMARK/extract$FUNCTION-$CFG_NAME.log"
...
then
    ...
    nohup sh -c 'python -u run_extract.py ${FUNCTION} ${CONFIG}' > $OUT 2>&1 &
else
	# 
    sbatch <<EOT
#!/bin/bash
...
python -u run_extract.py $FUNCTION $CONFIG
EOT
fi