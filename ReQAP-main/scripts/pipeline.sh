...
FUNCTION=$1
CONFIG=${2:-"config/perqa/reqap.yml"}
PERSONA=${3:-""}
...
OUT="logs/$BENCHMARK/pipeline$FUNCTION-$CFG_NAME$PERSONA.log"
...
then
    ...
    nohup sh -c 'python -u pipeline.py ${FUNCTION} ${CONFIG}' > $OUT 2>&1 &
else
	# 
    sbatch <<EOT
#!/bin/bash
...
python -u pipeline.py $FUNCTION $CONFIG $PERSONA
EOT
fi