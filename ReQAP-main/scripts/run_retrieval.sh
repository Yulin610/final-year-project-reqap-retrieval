...
OUT="logs/$BENCHMARK/retrieval$FUNCTION-$CFG_NAME.log"
...
then
    ...
    nohup sh -c 'python -u run_retrieval.py ${FUNCTION} ${CONFIG}' > $OUT 2>&1 &
else
	# 
    sbatch <<EOT
#!/bin/bash
...
python -u run_retrieval.py $FUNCTION $CONFIG $PERSONA
EOT
fi