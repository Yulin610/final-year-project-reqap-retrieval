...
class BM25Index:
    ...
    corpus_text_to_id: Dict[str, int]
...
class BM25Retriever(Retriever):
    ...
    def build(self, index_dir: str, *, show_progress: bool = True) -> None:
        ...
        self._index = BM25Index(bm25=bm25, doc_ids=doc_ids, corpus_texts=corpus_texts, corpus_text_to_id=corpus_text_to_id)
    ...
    def load(self, index_dir: str) -> None:
        ...
        corpus_texts = list(bm25.corpus or [])
        if corpus_texts and len(corpus_texts) != len(doc_ids):
            raise ValueError(
                f"BM25 corpus/doc_ids length mismatch: corpus={len(corpus_texts)} doc_ids={len(doc_ids)} "
        ...
        if corpus_texts:
            for did, txt in zip(doc_ids, corpus_texts):
                corpus_text_to_id[str(txt)] = int(did)

        self._index = BM25Index(
            bm25=bm25,
            doc_ids=doc_ids,
            corpus_texts=corpus_texts,
            corpus_text_to_id=corpus_text_to_id,
        )
    ...
    def retrieve(self, query: str, *, top_k: int, threshold: float = 0.0) -> List[RetrievedDoc]:
        ...
        # IMPORTANT: `corpus` must be the same string corpus used at indexing time.
        # Passing integer doc_ids here breaks bm25s' document identity mapping and yields nonsense rankings.
        res = self._index.bm25.retrieve(
            [q_tokens],
            corpus=self._index.corpus_texts,
            k=top_k,
            sorted=True,
            show_progress=False,
        )

        raw_docs = res.documents[0].tolist()
        scores: Sequence[float] = [float(x) for x in res.scores[0].tolist()]

        out: List[RetrievedDoc] = []
        for raw_doc, score in zip(raw_docs, scores):
            if score < threshold:
                continue
            # bm25s may return either doc ids (ints) or corpus strings depending on version/config.
            if isinstance(raw_doc, (int, float)) or (isinstance(raw_doc, str) and raw_doc.isdigit()):
                doc_id = int(raw_doc)
            else:
                key = str(raw_doc)
                if key in self._index.corpus_text_to_id:
                    doc_id = int(self._index.corpus_text_to_id[key])
                else:
                    # Slow but safe fallback (dev workspaces are small)
                    try:
                        pos = self._index.corpus_texts.index(key)
                    except ValueError:
                        continue
                    doc_id = int(self._index.doc_ids[pos])
