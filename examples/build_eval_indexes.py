...
# 保证可以导入原始 ReQAP 代码和本模块（避免写死盘符，Linux/容器与 Windows 共用）
_EXAMPLES = Path(__file__).resolve().parent
_MODULAR = _EXAMPLES.parent
sys.path.insert(0, str(_EXAMPLES))

from perqa_benchmark_paths import reqap_main_root

_REQAP = reqap_main_root()
for _p in (_REQAP, _MODULAR):
    _s = str(_p)
...
_DEFAULT_RETRIEVE_ROOT = _REQAP / "data" / "retrieve"
WORKSPACE = os.environ.get(
    "RETRIEVE_EVAL_WORKSPACE",
    str(_DEFAULT_RETRIEVE_ROOT / "eval_workspace"),
)
DEV_JSONL = os.environ.get(
    "RETRIEVE_DEV_JSONL",
    str(_DEFAULT_RETRIEVE_ROOT / "dev_data.jsonl"),
)
...
LOCAL_DENSE_MODEL = _REQAP / "data" / "data" / "models" / "perqa" / "dense_adapted_phase1"
# Optional local SPLADE checkpoint (sibling of repo); overridden by PERQA_SPLADE_MODEL_TYPE_OR_PATH
_DEFAULT_LOCAL_SPLADE = _REQAP.parent / "splade_adapted_hn_phase1"
